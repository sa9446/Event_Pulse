...
enum class NoisePayloadType(val value: UByte) {
    ...
    PEER_STATE(0x21u),
    /** Real-time call control (ride the encrypted Noise payload path). */
    CALL_INVITE(0x30u),     // Outgoing call request: {callId, callerNickname, video}
    CALL_ACCEPT(0x31u),     // Callee accepted: {callId}
    CALL_REJECT(0x32u),     // Callee declined / busy: {callId}
    CALL_END(0x33u);        // Either side hung up / cancelled: {callId}
...
/**
 * Real-time call signal exchanged inside a [NoisePayload] of type CALL_*. The payload
 * data is a tiny JSON document (Gson) so it stays under the single-fragment size.
 */
data class CallSignal(
    /** Unique call id, generated by the caller. */
    val callId: String,
    /** Caller's nickname (INVITE only). */
    val nickname: String = "",
    /** True when the caller asked for video (INVITE only). */
    val video: Boolean = false
) {
    fun toJson(): String = com.google.gson.Gson().toJson(this)

    companion object {
        fun fromJson(json: String): CallSignal? = try {
            val parsed = com.google.gson.Gson().fromJson(json, CallSignal::class.java)
            if (parsed.callId.isBlank()) null else parsed
        } catch (_: Exception) {
            null
        }
    }
